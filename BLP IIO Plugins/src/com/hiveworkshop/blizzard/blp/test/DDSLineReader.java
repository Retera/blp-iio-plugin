package com.hiveworkshop.blizzard.blp.test;
...
public class DDSLineReader {
	...
	public byte[] readAll(ImageInputStream stream, BLPStreamMetadata.SampleType format, int bitCount, int width, int height) throws IOException{
		byte[] bytes;
		switch (format){
			case DXT1:
				width = fixSize(width);
			...
			case DXT5:
				...
				break;
			default:
				throw new IOException(format+" is not a supported format!");
		}
		return bytes;
	}
	...
	public void decodeLine(byte[] bytes, BLPStreamMetadata.SampleType format, byte [][] banks, int width, int y) throws IOException{
		switch (format){
			case DXT1:
			case DXT3:
			case DXT5:
				decodeDXT(bytes, format, banks, width, y);
				break;
			default:
				throw new IOException(format+" is not a supported format!");
		}
	...
	private void decodeDXT(byte[] bytes, BLPStreamMetadata.SampleType format, byte [][] banks, int width, int y) throws IOException{
		...
		if (lineNumber == 0){
			...
			} else if (format == SampleType.DXT5){
				for (int x = 0; x < fixedWidth; x = x + 4) {
